...
const L1MIAGE = () => {
    ...
    const { 
        ...
        isUpdating,
        isDeleting
    } = useEtudiants('L1', 'MIAGE');

    // Charger les préférences depuis localStorage
    useEffect(() => {
        const savedSearchTerm = localStorage.getItem('l1MIAGE_searchTerm');
        const savedSortOption = localStorage.getItem('l1MIAGE_sortOption');
    ...
    useEffect(() => {
        localStorage.setItem('l1MIAGE_searchTerm', searchTerm);
    }, [searchTerm]);

    useEffect(() => {
        localStorage.setItem('l1MIAGE_sortOption', sortOption);
    }, [sortOption]);
    ...
    const handleClearFilters = () => {
        ...
        localStorage.removeItem('l1MIAGE_searchTerm');
        localStorage.removeItem('l1MIAGE_sortOption');
        setCurrentPage(1);
    ...
    if (error) {
        return (
            <div className="space-y-6">
                <PageHeader 
                    title="L1 MIAGE"
                    subtitle="Gestion des étudiants en Licence 1 Méthode Informatique Appliquée à la gestion d'entreprise"
                />
    ...
    return (
        <>
                    ...
                    <div className="bg-slate-900/60 backdrop-blur-sm rounded-xl border border-blue-400/20 p-4">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-white font-medium">Liste des étudiants L1 MIAGE</h3>
                                    <p className="text-blue-200/60 text-sm">
                                        {searchTerm ? (
                                            ...
                                            `${etudiants.length} étudiant${etudiants.length !== 1 ? 's' : ''} en L1 MIAGE`
                                        )}
                                    </p>
                                </div>
                            </div>
                        ...
                        {searchTerm ? (
                            ...
                            <>
                                ...
                                <motion.p 
                                    ...
                                    transition={{ delay: 0.3 }}
                                >
                                    Aucun étudiant n'a été importé en L1 MIAGE. Importez un fichier Excel pour commencer.
                                </motion.p>
            ...
            <ImportEtudiantModal
                ...
                niveau="L1"
                filiere="MIAGE"
                titre="Importer les étudiants L1 MIAGE"
            />
