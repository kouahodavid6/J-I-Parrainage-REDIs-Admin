...
const L2MIAGE = () => {
    ...
    const { 
        ...
        isUpdating,
        isDeleting
    } = useEtudiants('L2', 'MIAGE');

    // Charger les préférences depuis localStorage
    useEffect(() => {
        const savedSearchTerm = localStorage.getItem('l2MIAGE_searchTerm');
        const savedSortOption = localStorage.getItem('l2MIAGE_sortOption');
    ...
    useEffect(() => {
        localStorage.setItem('l2MIAGE_searchTerm', searchTerm);
    }, [searchTerm]);

    useEffect(() => {
        localStorage.setItem('l2MIAGE_sortOption', sortOption);
    }, [sortOption]);
    ...
    const handleClearFilters = () => {
        ...
        localStorage.removeItem('l2MIAGE_searchTerm');
        localStorage.removeItem('l2MIAGE_sortOption');
        setCurrentPage(1);
    ...
    if (error) {
        return (
            <div className="space-y-6">
                <PageHeader 
                    title="L2 MIAGE"
                    subtitle="Gestion des étudiants en Licence 2 Méthode Informatique Appliquée à la gestion d'entreprise"
                />
    ...
    return (
        <>
                    ...
                    <div className="bg-slate-900/60 backdrop-blur-sm rounded-xl border border-blue-400/20 p-4">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-white font-medium">Liste des étudiants L2 MIAGE</h3>
                                    <p className="text-blue-200/60 text-sm">
                                        {searchTerm ? (
                                            ...
                                            `${etudiants.length} étudiant${etudiants.length !== 1 ? 's' : ''} en L2 MIAGE`
                                        )}
                                    </p>
                                </div>
                            </div>
                        ...
                        {searchTerm ? (
                            ...
                            <>
                                ...
                                <motion.p 
                                    ...
                                    transition={{ delay: 0.3 }}
                                >
                                    Aucun étudiant n'a été importé en L2 MIAGE. Importez un fichier Excel pour commencer.
                                </motion.p>
            ...
            <ImportEtudiantModal
                ...
                niveau="L2"
                filiere="MIAGE"
                titre="Importer les étudiants L2 MIAGE"
            />
